...
proc write_deal {} {
  # note a lot of variables are called "spades" because of what they were adapted, assume "spades" means suit or hearts
  set tr [deal::tricks north notrump]
  set trS [deal::tricks north hearts]

  set scV [score {3 notrump} vul $tr]
  set scSV [score {4 hearts} vul $trS]
  set scNV [score {3 notrump} nonvul $tr]
  set scSNV [score {4 hearts} nonvul $trS]
...
#  puts "There are $trS tricks in hearts (NV $scSNV, V $scSV); imps V $impSV; imps NV $impSNV"
#  puts "---------------------------------"
}
...
deal_finished {
  ...
  puts "Across all hands there were an average of [trickScount average] tricks available in hearts"
  puts "=================================================="
  set sameS [expr $count(hands) - $count(choiceOfSpBetter) - $count(choiceOfSpWorse)]
  puts "If we choice of game between 4 Hearts and 3nt:"
  puts "Hearts scores better $count(choiceOfSpBetter) times ([expr double(round(double($count(choiceOfSpBetter))/$count(hands)*10000))/100]%), NT scores better $count(choiceOfSpWorse) times ([expr double(round(double($count(choiceOfSpWorse))/$count(hands)*10000))/100]%) and they are the same $sameS times ([expr double(round(double($sameS)/$count(hands)*10000))/100]%)"
  puts "So in MP of even split H scores [expr double(round(double($count(choiceOfSpBetter)*2+$sameS)/(2*$count(hands))*10000))/100]% and NT scores [expr double(round(double($count(choiceOfSpWorse)*2+$sameS)/(2*$count(hands))*10000))/100]%"
  puts "=================================================="
  puts "In terms of games: both games make $count(choiceOfBothMake) times ([expr double(round(double($count(choiceOfBothMake))/$count(hands)*10000))/100]%), only the NT game makes $count(choiceOf3ntMake) times ([expr double(round(double($count(choiceOf3ntMake))/$count(hands)*10000))/100]%), only hearts make $count(choiceOfSpMake) times ([expr double(round(double($count(choiceOfSpMake))/$count(hands)*10000))/100]%), and both are down $count(choiceOfBothFail) times ([expr double(round(double($count(choiceOfBothFail))/$count(hands)*10000))/100]%)"
  puts "So being in hearts scored [impSpadeNV average] imps better when NV"
  puts "So being in hearts scored [impSpadeV average] imps better when V"
}
...
main {
  ...
  # West passes in 1st seat
  reject if {[spades north] != 2}
  reject if {[hearts north] != 4}
  reject if {[diamonds north] != 3}
  reject if {[clubs north] != 4}
  reject if {[hcp north]<10 || [hcp north]>12}
  # North is 2=4=3=4 and 10-12
  reject if {![passover1nt east]}
  # East must pass over 1NT
  # South is AQ9 AQT6 984 K53 as given at top of file
  accept
}
